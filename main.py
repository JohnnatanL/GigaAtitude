...
def main():
    if st.session_state.get("authenticated"):
        ...
        if st.session_state.get("role") == "admin":
            pg = st.navigation({"Inicio": [home, gerenc_users, forms_consultor, rel_gestor]})
            pg.run()

        elif st.session_state.get("role") == "planejamento":
            pg = st.navigation({"Inicio": [home, gerenc_users, rel_gestor]})
            pg.run()

        elif st.session_state.get("role") == "gestao":
            pg = st.navigation({"Inicio": [home, forms_consultor, rel_gestor]})
            pg.run()
